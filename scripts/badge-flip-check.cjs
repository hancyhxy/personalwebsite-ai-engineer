 ...
 try{
  ...
  for(const mobile of [false,true]){
   ...
   if(!mobile){
    ...
    assert.equal(await page.locator('.scene-fallback a').count(),16);assert.equal(await page.locator('.project-flip-layer').count(),0);
  ...
  assert.equal(await reduced.locator('.scene-fallback a').count(),16);assert.notEqual(await reduced.evaluate(()=>getComputedStyle(document.body).overflow),'hidden');await reduced.close();
  ...
  await failure.close();console.log('PASS all 16 canonical full-resolution Heroes load and template identities are unchanged');
