...
for i, p in enumerate(projects):
    rows.append(f'<a class="work-item" href="{escape(urljoin(BASE, p["projectUrl"]))}"><span class="work-number micro">{i+1:02}</span><div class="work-thumb"><img src="{escape(urljoin(BASE, p["coverImageUrl"]))}" alt="" width="160" height="120" loading="lazy"></div><div class="work-copy"><h3>{escape(p["project name"])}</h3><p>{escape(p["tag"])}</p></div><span class="work-meta micro">{escape(p["date"][:4])}</span><span class="work-arrow" aria-hidden="true">↗</span></a>')
...
for url in [BASE] + [urljoin(BASE, p['projectUrl']) for p in projects]:
...
lines += [f'- [{p["project name"]}]({urljoin(BASE, p["projectUrl"])}): {p["tag"]}.' for p in projects]
