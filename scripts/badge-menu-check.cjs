...
(async () => {
  ...
  try {
    for (const config of [
      ...
      console.log('PASS', name, 'menu visibility, keyboard trap/Escape, 16 projects, internal scrolling, sort+scroll return through Next, chapter navigation, clean résumé handoff');
      await page.close();
    }
    ...
    assert.equal(await failure.locator('#work-collection .work-item[href*="project-scrollcarousel.html"]').count(), 16);
    await failure.keyboard.press('Escape');
    await failure.waitForFunction(() => getComputedStyle(document.body).overflow !== 'hidden');
    console.log('PASS failed gallery retains 16 local detail links, legacy archive hash and Escape unlock');
    await failure.close();
