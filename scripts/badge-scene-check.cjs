...
(async()=>{
  ...
  try {
    ...
    const contract=await page.evaluate(()=>({type:BadgeScene.runtime.rig.camera.type,total:BadgeScene.projects.length,groups:BadgeScene.groups.map(g=>g.projects.length),independent:BadgeScene.groups[0].projects.map(i=>BadgeScene.projects[i].title),featured:BadgeScene.featured,archived:BadgeScene.archived,duration:BadgeScene.duration,heroOverlap:BadgeScene.timing.heroOverlap,physicalHeight:document.querySelector('#work-story').getBoundingClientRect().height/innerHeight,blanks:BadgeScene.runtime.background.mesh.isInstancedMesh,old:[...document.scripts].some(s=>/badge-webgl|badge-scroll-stage/.test(s.src))}));
    assert.equal(contract.type,'PerspectiveCamera');assert.equal(contract.total,18);assert.deepEqual(contract.groups,[4,4,3]);assert.deepEqual(contract.independent,['Tech Fest | AI Showcase','Claude Code ↔ Figma','Interactive Virtual Drum Kit','AI-Assisted Video Editing Workflow']);assert.equal(contract.featured.length,11);assert.deepEqual(contract.archived,[2,6,10,11,13,14,15]);assert.equal(contract.old,false);assert.equal(contract.blanks,true);assert.ok(contract.duration<=4);assert.ok(Math.abs(contract.physicalHeight+contract.heroOverlap-contract.duration)<.002);
    pass('Three overview chapters, 11 featured / 7 archived, <=4 viewport travel; no legacy driver');
    ...
    const missing=await browser.newPage();watch(missing);await missing.route('**/assets/images/thumbs/Rider-Dispatch-Scheduling-Platform-thumb.jpg',r=>r.abort());await missing.goto(base+'/'+entry);await missing.waitForSelector('.scene-fallback-mode');assert.equal(await missing.locator('.scene-fallback a').count(),18);assert.notEqual(await missing.evaluate(()=>getComputedStyle(document.body).overflow),'hidden');await missing.close();
