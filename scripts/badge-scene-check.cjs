...
(async()=>{
  ...
  try {
    ...
    await page.locator('#resume-link').click();await page.waitForFunction(()=>typeof document.querySelector('#resume-frame').contentWindow?.resumePressSave==='function',null,{timeout:20000});
    const printer=page.frameLocator('#resume-frame');await printer.locator('#printer-stage[data-state="ready"]').waitFor({timeout:20000});
    ...
    pass('Learn more still auto-prints the résumé and fits its iframe');
    ...
    const downloadPromise=page.waitForEvent('download',{timeout:20000});await page.locator('.resume-download-primary').click();const download=await downloadPromise;assert.ok(/\.pdf$/i.test(download.suggestedFilename()));await download.delete();
    ...
    pass('Résumé download and printer replay preserved');
    ...
    pass('Opening uses 11 curated images, 16:9 throughout, synchronized landing without geometry jump');await intro.close();
