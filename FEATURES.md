...
- On touch phones at ≤700px, local-only device-orientation input starts automatically when the browser already permits it. In a secure context, check existing permission once when the badge becomes visible; browsers that require a new permission prompt must fall back to an explicit `Enable tilt interaction` click. Never bypass activation requirements, repeatedly retry, auto-prompt, or upload sensor values. Touch dragging no longer drives the portrait or captures native scrolling.
